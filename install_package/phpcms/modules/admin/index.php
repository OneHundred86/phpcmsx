...
class index extends admin {
	...
	public function admin() {
		if(isset($_GET['dosubmit'])) {
			...
			if(!$r) showmessage(L('user_not_exist'),'?m=admin&c=index&a=admin');
			...
			if($r['password'] != $password) {
				...
				showmessage(L('password_error',array('times'=>$times)),'?m=admin&c=index&a=admin',3000);
			}
	...
	public function public_card() {
		...
		if (isset($_GET['dosubmit'])) {
			...
			header("location:?m=admin&c=index&a=admin&dosubmit=1&card=1");
			exit;
		}
		pc_base::load_app_class('card', 'admin', 0);
	...
	public function public_logout() {
		...
		showmessage(L('logout_success').$phpsso_logout,'?m=admin&c=index&a=admin');
	}
