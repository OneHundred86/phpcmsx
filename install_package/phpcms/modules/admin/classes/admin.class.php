...
class admin {
	...
	final public function check_admin() {
		if(ROUTE_M =='admin' && ROUTE_C =='index' && in_array(ROUTE_A, array('admin', 'public_card'))) {
			return true;
		} else {
	...
	final public function check_priv() {
		if(ROUTE_M =='admin' && ROUTE_C =='index' && in_array(ROUTE_A, array('admin', 'init', 'public_card'))) return true;
	...
	final private function manage_log() {
		...
		extract($setconfig);
 		if($admin_log==1){
 			$action = ROUTE_A;
//var_dump(__METHOD__, ROUTE_M, ROUTE_C, ROUTE_A);
...
//var_dump(compact('ip', 'userid', 'username', 'url', 'time'));
				$res = $log->insert(array(
				    'module'=>ROUTE_M,
                    'username'=>$username,
                    'userid'=>$userid,
                    'action'=>ROUTE_C,
                    ...
                    'time'=>$time,
                    'ip'=>$ip,
                    'data' => mb_strcut(json_encode($_REQUEST), 0, 65535),
                ));
//var_dump(compact('res'));die();
//			}
	  	}
	}
	...
	final private function lock_screen() {
		if(isset($_SESSION['lock_screen']) && $_SESSION['lock_screen']==1) {
			...
			showmessage(L('admin_login'),'?m=admin&c=index&a=admin');
		}
	}
...
//var_dump($_REQUEST['pc_hash'], $_SESSION);die();
